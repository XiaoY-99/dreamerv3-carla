...
from dreamerv3 import agent as base_agent
...
class Agent(embodied.Agent):
  ...
  def __new__(subcls, obs_space, act_space, config):
    ...
    model = base_agent.Agent(obs_space, act_space, config)

    outer = super().__new__(Agent)
    outer._init_outer(model, obs_space, act_space, config, jaxcfg)
    return outer

  def __init__(self, *args, **kwargs):
    # Prevent double initialization by Python after __new__
    pass

  def _init_outer(self, model, obs_space, act_space, config, jaxcfg):
    print("Initializing JAX Agent wrapper... setting train_lock, policy_lock, etc.")
