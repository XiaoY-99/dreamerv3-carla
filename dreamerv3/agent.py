...
def imag_loss(
  ...
  entropies = list(ents.values())
  if entropies:
      ent_sum = jnp.sum(jnp.stack(entropies, axis=0))
  else:
      ent_sum = jnp.array(0.0, dtype=jnp.float32)

  actent = jnp.array(actent, dtype=jnp.float32)

  policy_loss = logpi * sg(adv_normed) + actent * ent_sum

  '''
  policy_loss = sg(weight[:, :-1]) * -(
      #logpi * sg(adv_normed) + actent * sum(ents.values()))
      logpi * sg(adv_normed) + actent * jnp.sum(jnp.stack(list(ents.values())))
  )
  '''
